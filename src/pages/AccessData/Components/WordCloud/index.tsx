    ...
    series: [
      {
        ...
        colors: [AnalyticsColors.ecWine, AnalyticsColors.grey, AnalyticsColors.purple, AnalyticsColors.darkGray],
      },
    ],
    tooltip: {
      useHTML: true,
      formatter() {
        ...
        return `<div style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 10px; z-index: 1">
                <h1 style="font-size: 30px; font-family: Inter; color: ${AnalyticsColors.black}; margin: 0px;"> ${
          self.point.options.weight
        }</h1>
                <span style="font-size: 16px; font-weight: 500; font-style: normal; color: ${
                  AnalyticsColors.darkGray
                }"> ${self.point.options.weight === 1 ? 'Menção' : 'Menções'} </span>
                </div>`;
      },
    },
  };
